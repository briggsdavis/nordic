                  ...
                  <SelectContent>
                    ...
                    {availableOptions.map((option) => (
                      <SelectItem key={option.id} value={option.id}>
                        {option.name} -{" "}
                        {formatPrice(option.price_per_unit ?? product.price_per_unit)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                  ...
                  <SelectContent>
                    {variants.map((variant) => (
                      <SelectItem key={variant.value} value={variant.value}>
                        {variant.label} - {formatPrice(effectivePricePerUnit * variant.weight)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            <div className="space-y-2 border-t pt-6">
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">
                  {product.allow_size_selection ? selectedVariant : "unit"} × {quantity}
                </span>
