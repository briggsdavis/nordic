...
export const useCart = () => {
  ...
  const addToCart = useMutation({
    mutationFn: async ({ productId, variant, quantity = 1 }: { productId: string; variant: string; quantity?: number }) => {
  ...
  const cartTotal = cartItems.reduce((total, item) => {
    const price = item.product ? getVariantPrice(item.variant, item.product.price_per_kg) : 0;
