...
interface OrdersTabProps {
  adminOrders: UseAdminOrdersResult
  expandedOrderId: string | null
  onExpandedChange: (id: string | null) => void
}

export const OrdersTab = ({ adminOrders, expandedOrderId, onExpandedChange }: OrdersTabProps) => {
  const {
    orders,
    pendingReviewOrders,
    inTransitOrders,
        ...
        {orderList.length === 0 ? (
          ...
          orderList.map((order) => (
            <OrderRow
              ...
              onReject={(reason) => rejectPayment.mutate({ orderId: order.id, reason })}
              onSaveNote={(note) => saveNote.mutate({ orderId: order.id, note })}
