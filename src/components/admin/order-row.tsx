...
import type { OrderWithProfile } from "@/hooks/use-admin-orders"
            ...
            <DropdownMenuContent align="start">
              ...
              {STATUS_OPTIONS.map(({ value, label }) => (
                <DropdownMenuItem
                  ...
                  className={cn(value === order.status && "opacity-50 cursor-not-allowed")}
                >
                  {label}
                </DropdownMenuItem>
              ))}
                  ...
                  <div className="space-y-2">
                    ...
                    {order.profile?.email && (
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-muted-foreground">{order.profile.email}</span>
                      </div>
                    )}
                    {order.contact_phone && (
                      <div className="flex items-center gap-2 text-sm">
                        <Phone className="h-4 w-4 flex-shrink-0 text-muted-foreground" />
                        <span className="text-muted-foreground">{order.contact_phone}</span>
                      </div>
                    )}
                  </div>
                </div>
