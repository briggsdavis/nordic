...
export const ShipmentTimeline = ({
  ...
  const { data: definitions, isLoading: defsLoading } = useShipmentStageDefinitions()
  ...
  const paymentVerifiedActive =
    orderStatus !== "verifying" && orderStatus !== "rejected" && orderStatus !== undefined
      ...
      <div className="flex items-start pb-2">
        ...
        <div className="flex flex-col items-center">
          <div
            className={cn(
              ...
              paymentVerifiedActive ? "bg-blue-500 text-white" : "bg-muted text-muted-foreground",
            )}
          >
            {paymentVerifiedActive ? <Check className="h-4 w-4" /> : <Clock className="h-4 w-4" />}
          </div>
          <div className="mt-2 w-24 text-center">
            <p
              className={cn(
                "text-xs font-medium leading-tight",
                paymentVerifiedActive ? "text-blue-600" : "text-muted-foreground",
              )}
            >
              Payment Verified
            </p>
        ...
        {hasRealStages
          ? stages!.map((stage, index) => {
              ...
              const lineIsActive =
                index === 0 ? paymentVerifiedActive : stages![index - 1].status === "completed"
